...
# fi
echo Executing ./doAnalysis -i $INPUTFILENAMES -t Events -o ${OUTPUTNAME}.root ${EXTRAARGS}
./doAnalysis -i $INPUTFILENAMES -t Events -o ${OUTPUTNAME}.root ${EXTRAARGS}
RET=$?
...
if [ ${RET} != 0 ]; then
    if [[ "${EXTRAARGS}" = *"ignorebadfiles"* ]]; then
        ...
        echo "Removing output file because ./doAnalysis returned exit code ${RET}"
        rm ${OUTPUTNAME}.root
    fi
fi
